...
export const compareUrl = (url, range) => `${sanitizeGitHubUrl(url)}/compare/${range}`;
export const commitUrl = (url, commit) => `${sanitizeGitHubUrl(url)}/commit/${commit}`;
export function gitLogFromRange(range) {
    try {
        const logOut = execSync(`git log --no-merges --date-order --format="%H%x09%s" ${range}`);
        const lines = logOut.toString().trim().split("\n");
        const shaMessagePairs = lines.map((line) => line.split("\t"));
        return shaMessagePairs;
    }
    catch (err) {
        console.warn(`git log failed on range ${range}, one of those tags probably does not exist`);
        return [];
    }
}
