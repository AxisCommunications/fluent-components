...
export function release(increment) {
    ...
    if (releaseType === undefined) {
        throw new Error(`unknown increment: ${increment}, expected one of ${semver.RELEASE_TYPES}`);
    }
    const { version, repository } = JSON.parse(readFileSync("package.json").toString());
    const isPre = releaseType.startsWith("pre");
    const nextVersion = semver.inc(version, releaseType, isPre ? "alpha" : undefined);
    if (nextVersion === null) {
