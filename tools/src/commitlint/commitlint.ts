...
const re =
  /^(?<type>build|chore|ci|docs|feat|fix|perf|refactor|release|revert|style|test)(?<scope>\(\w+\)?((?=:\s)|(?=!:\s)))?(?<breaking>!)?(?<subject>:\s.*)?|^(?<merge>Merge \w+)$/gm;

function isConventionalCommitMessage(msg: string) {
  const match = msg.match(re);
  return match !== null && match[0] === msg;
}

function log(range: string) {
  return execSync(`git rev-list ${range}`).toString().trim().split("\n");
}

function message(sha: string) {
...
export function messageLint(msg: string) {
  let rc = 0;
